...
def getSystematics(central, diff, title='data'):
    ...
    for sys in diff:
        ...
        for j in range(Ntot):
            if (sys.getY(j) > 0):
                yup[j] = yup[j] + sys.getY(j)*sys.getY(j)
            if (sys.getY(j) < 0):
                ydown[j] = ydown[j] + sys.getY(j)*sys.getY(j)
...
def Print(h1):
    ...
    print ("# bin, edge, width, center, hight, error") 
    for i in range(h1.GetNbinsX()):
        y = h1.GetBinContent(i+1)
        ey = h1.GetBinError(i+1)
        x = h1.GetBinCenter(i+1)
        x1 = h1.GetBinLowEdge(i+1)
...
def TH1correctError(h1):
    for i in range(h1.GetNbinsX()):
        ...
        # h1.SetBinError(i+1,ey)
        if (y > 0):
            h1.SetBinError(i+1, math.sqrt(y))
        else:
            h1.SetBinError(i+1, 0)

def getMaxNonzero(h1, ycut=0.5):
    tmp=0
    for i in range(h1.GetNbinsX()-1, -1, -1):
        y = h1.GetBinContent(i+1)
        x=h1.GetBinCenter(i+1)
        err = h1.GetBinWidth(i)
        #print(i,x,y)
        if (y > ycut):
                  print("Find maximum=",i,x+err,y)
                  tmp=x+err;
                  break
    return tmp
...
# remove bins with large errors (for representations)
# only consider points with <75% of the relative error
def TH1RemoveBinsWithLargeErrors(h1, hmax=0.75):
    for i in range(h1.GetNbinsX()):
        y = h1.GetBinContent(i+1)
        ey = h1.GetBinError(i+1)
        x = h1.GetBinCenter(i+1)
...
def TGraphErrorsSubtract(g1, g2):
    ...
    for i in range(g1.GetN()):
        ...
        eh = 0.
        if (y1 != 0. and y2 != 0.):
            el = math.sqrt(dy1l*dy1l+dy2l*dy2l)
        if (y1 != 0. and y2 != 0.):
            eh = math.sqrt(dy1h*dy1h+dy2h*dy2h)
...
def TGraphErrorsDivide(g1, g2, mode=0, style=0):
    ...
    for i in range(min):
        ...
        eh = 0.
        if (y1 != 0. and y2 != 0.):
            el = TMath.Sqrt(dy1l*dy1l+dy2l*dy2l)*(y1/y2)
        if (y1 != 0. and y2 != 0.):
            eh = TMath.Sqrt(dy1h*dy1h+dy2h*dy2h)*(y1/y2)
...
def TGraphMergeAtPoint(g1, g2, point):
...
def TH1divTH1(h1, h2):
    ...
    # print xx
    ratio = TH1F(h1.GetTitle()+"_div_"+h2.GetTitle(),
                 h1.GetTitle()+"_div_"+h2.GetTitle(), Ntot-1, xx)
    for i in range(Ntot):
...
# get maximum of the filled bin
def getMaxTH1(h1):
    xaxis = h1.GetXaxis()
    Ntot = xaxis.GetNbins()+1
    for i in range(Ntot-2, 0, -1):
        y1 = h1.GetBinContent(i+1)
        e1 = h1.GetBinError(i+1)
        x1 = h1.GetBinCenter(i+1)
        ex1 = h1.GetBinWidth(i+1)
...
# Make a braph representing errors
def TGraphErrorsInstead(g1):
    g3 = TGraphAsymmErrors()
    n1 = g1.GetN()
    x1 = 0.
    y1 = 0.
...
def TGraphErrorsDivideInv(g1, g2):
    ...
    for i in range(g1.GetN()):
        ...
        eh = 0.
        if (y1 != 0. and y2 != 0.):
            eh = TMath.Sqrt(dy1l*dy1l+dy2l*dy2l)*(y2/y1)
        if (y1 != 0. and y2 != 0.):
            el = TMath.Sqrt(dy1h*dy1h+dy2h*dy2h)*(y2/y1)
        g3.SetPointError(i, dx1h, dx1l, el, eh)
...
def getH1D(file, name, style=1, col=21, marker_size=0.2, xlab="none", ylab="none"):
    ...
    ax.SetLabelOffset(.015)
    ay.SetLabelOffset(.015)
    return h1, ax, ay


# write histogram h1 to text file
def writeH1D(filename, h1):
    xfile = open(filename, "w")
    s = "# "+str(h1.GetName())+" created on: "+str(datetime.now())+"\n"
    print(s)
    xfile.write(s)
    xfile.write("# x(center)  y(center)  dy\n")
    for i in range(h1.GetNbinsX()):
        y = h1.GetBinContent(i+1)
        ey = h1.GetBinError(i+1)
        x = h1.GetBinCenter(i+1)
        ex = h1.GetBinWidth(i+1)/2.0
        s = str(x)+" "+str(y)+" "+str(ey)+" \n"
        xfile.write(s)
    xfile.close()
...
# ROC curve
def getROC(sigHist, bkgHist, color=1, style=20, size=0.9):
...
#  each y point is this loop's slice integral over total integral (background)
    for i in range(nbins):
        ...
        sig_slice_integral = sigHist.Integral(nbins-i, nbins)
        bkg_slice_integral = bkgHist.Integral(nbins-i, nbins)
        x = sig_slice_integral/sig_integral
        y = bkg_slice_integral/bkg_integral

        ex = math.sqrt(sig_slice_integral)/sig_integral
        ey = math.sqrt(bkg_slice_integral)/bkg_integral
