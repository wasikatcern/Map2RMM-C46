...
def TH1correctError(h1):
 for i in range(h1.GetNbinsX()):
     ...
     #h1.SetBinContent(i+1,y)
     #h1.SetBinError(i+1,ey)
     if (y>0): h1.SetBinError(i+1,math.sqrt(y))
     else: h1.SetBinError(i+1,0)
...
def TGraphAbove1(g1):
  ...
  for i in range(g1.GetN()):
    ...
    g.SetPoint(i,x1,y1);
    g.SetPointError(i,dx1l,dx1h,dy1l,dy1h);
    if (y1>1): xsum1=xsum1+y1 -1 
    if (y1<1): xsum2=xsum2+1-y1 
    nn=nn+1 
  if (nn==0): return 0
...
def rms90(h):
   ...
   x = 0
   if (sumw != 0): x=sumwx/sumw;
   rms2 =0
   if (sumw != 0): rms2 = TMath.Abs(sumwx2/sumw -x*x)
